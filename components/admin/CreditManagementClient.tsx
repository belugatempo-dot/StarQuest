      ...
      <div className="dark-card rounded-lg shadow-md p-6">
        ...
        {children.length === 0 ? (
          <div className="text-center py-8 bg-white/5 rounded-lg">
            <p className="text-slate-400">{t("credit.noChildren")}</p>
          </div>
        ) : (
              ...
              return (
                <div
                  ...
                  className={`border rounded-lg p-4 ${
                    creditEnabled ? "border-primary bg-primary/5" : "border-white/10"
                  }`}
                >
                  {/* Child Header */}
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center space-x-2">
                      <div className="w-10 h-10 bg-white/15 rounded-full flex items-center justify-center text-xl">
                        👤
                      </div>
                      <div>
                  ...
                  <div className="space-y-2 mb-4">
                    ...
                    {creditEnabled && (
                      <>
                        ...
                        <div className="flex justify-between text-sm">
                          <span className="text-slate-400">{t("credit.creditUsed")}:</span>
                          <span className={`font-medium ${creditUsed > 0 ? "text-warning" : ""}`}>
                            {creditUsed} ⭐
                          </span>
                        </div>
                        <div className="flex justify-between text-sm">
                          <span className="text-slate-400">{t("credit.availableCredit")}:</span>
                  ...
                  <button
                    ...
                    className="w-full px-4 py-2 bg-white/10 hover:bg-white/15 rounded-lg text-sm font-medium transition"
                  >
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-3">
              <select
                ...
                className="px-3 py-2 border border-blue-500/30 rounded-lg bg-white/5 text-blue-300 font-medium focus:ring-2 focus:ring-blue-400 focus:border-transparent disabled:opacity-50"
              >
              ...
              {savingSettlementDay && (
                <span className="text-sm text-blue-300">{t("common.saving")}</span>
              )}
            </div>
            {settlementDayError && (
