/**
 * Tests for reset-child-password admin API route
 * Tests POST from app/[locale]/api/admin/reset-child-password/route.ts
 */
...
import { POST } from "@/app/[locale]/api/admin/reset-child-password/route";
...
function makeRequest(body: any) {
  return new NextRequest("http://localhost/en/api/admin/reset-child-password", {
    method: "POST",
    body: JSON.stringify(body),
  });
}

describe("Reset Child Password API Route", () => {
  let userChain: any;
  ...
  it("returns 401 when not authenticated", async () => {
    ...
    const request = makeRequest({ childId: "child-1", newPassword: "newpass123" });
  ...
  it("returns 400 when childId missing", async () => {
    const request = makeRequest({ newPassword: "newpass123" });
    const response = await POST(request);
  ...
  it("returns 400 when password too short (< 6 chars)", async () => {
    const request = makeRequest({ childId: "child-1", newPassword: "abc" });
    const response = await POST(request);
  ...
  it("returns 404 when child not found", async () => {
    ...
    const request = makeRequest({ childId: "nonexistent", newPassword: "newpass123" });
    const response = await POST(request);
  ...
  it("returns 500 when RPC throws", async () => {
    mockRpc.mockResolvedValue({
      data: null,
      error: { message: "Reset function failed" },
    });
    ...
    expect(response.body.error).toBe("Reset function failed");
  });
  ...
  it("returns success on successful reset", async () => {
    ...
    expect(mockRpc).toHaveBeenCalledWith("admin_reset_child_password", {
      p_child_id: "child-1",
