...
export default async function DashboardPage({
  ...
  if (user.role === "parent") {
    return <ParentDashboard user={user} locale={locale} t={t} supabase={supabase} />;
  }

  return <ChildDashboard user={user} locale={locale} t={t} supabase={supabase} />;
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <Link href={`/${locale}/profile#credit`}>
          <div className="dark-card rounded-lg shadow-md p-6 hover:shadow-lg transition cursor-pointer">
            <div className="flex items-center justify-between mb-2">
              <h3 className="text-lg font-semibold text-slate-300">
                Credit Management
...
async function ChildDashboard({
  ...
  const [balanceResult, transactionsResult, levelsResult] = await Promise.all([
    supabase
      .from("child_balances")
      .select("*")
      .eq("child_id", user.id)
  ...
  const { data: recentTransactions } = transactionsResult;
